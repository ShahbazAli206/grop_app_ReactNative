...
import {Modal,  ImageBackground, Pressable, TouchableOpacity, Text,   StatusBar,  View, Image, StyleSheet,FlatList,  SafeAreaView } from "react-native";
...
function WishData() {
  ...
  const getWishList = (userid) => {
   axios.get(`/api/users/${userid}/wishlists`, {
     headers: { 'Content-Type' : 'application/json'}  })
    .then(response => { setDetails(response.data.data);})
    .catch(error => { console.log("opps error gaya while whishlist fetch data", error.response); })
  }

  useEffect(() => {
    getWishList(userid);
    const interval =  setInterval(() => { getWishList(userid);  }, 4000);
  ...
  return (
    <View style={{ flex: 1,      }}>
      ...
      <SubmitButton
        mode="contained"
        onPress={() => {AddToCart(); setModalVisible(!modalVisible)}}
      >
        Submit
      </SubmitButton>
      <Pressable
      ...
      {details && (
        <FlatList
          numColumns={2}
          data={details.filter((item, index, self) =>
